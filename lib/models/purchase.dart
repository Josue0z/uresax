...
class Purchase {
  ...
  Future<Map<String,dynamic>> create()async{
     try{
       var response = await httpClient.post('/purchases',data: toMap());
   ...
   static Future<List<Map<String,dynamic>>> getPurchases({String sheetId = ''})async{
     try{
      var results = await httpClient.get('/purchases?sheetId=$sheetId');
      return (results.data as List).map((e) => e).toList().cast<Map<String,dynamic>>();
     }catch(e){
      rethrow;
     }
   }
